...
def apply_gnome_theme(theme=None) -> bool:
    """
    Applies the theme in user theme extension if it is Obsidian flow and extension installed.
    """
    try:
        if theme is None:
...
def get_current_theme() -> str:
    """
    Throws an error if theme is not Obsidian flow.
    """
    try:
        output = subprocess.check_output(['dconf', 'read', config.user_themes_extension], text=True)
        output = output.strip().strip("'")

        if not output.startswith("Obsidian flow"):
            raise Exception(f"Theme {output} doesn't appear to be a Obsidian flow theme")
        return output
    except subprocess.CalledProcessError:
